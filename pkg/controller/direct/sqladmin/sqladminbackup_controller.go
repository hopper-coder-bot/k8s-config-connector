...
type sqlAdminBackupAdapter struct {
	...
	desired *krm.SQLAdminBackup
	actual  *api.BackupRun
...
func (a *sqlAdminBackupAdapter) Create(ctx context.Context, op *directbase.CreateOperation) error {
	...
	desiredProto := SQLAdminBackupSpec_ToProto(mapCtx, &a.desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	desiredAPI, err := toAPI(desiredProto)
	if err != nil {
		return err
	}
...
func (a *sqlAdminBackupAdapter) Update(ctx context.Context, op *directbase.UpdateOperation) error {
	...
	actualProto, err := toProto(a.actual)
	if err != nil {
		return err
	}
	...
	diff, err := compareResource(ctx, actualProto, desiredProto)
	if err != nil {
		return err
	}
...
func (a *sqlAdminBackupAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	mapCtx := &direct.MapContext{}
	spec := SQLAdminBackupSpec_FromProto(mapCtx, actualProto)
	if mapCtx.Err() != nil {
